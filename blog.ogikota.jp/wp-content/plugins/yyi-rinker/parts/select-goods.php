 ...
 * Time: 14:37
 *
 * When the media button push, The page open.
 */
		...
		function yyi_rinker_search_from_api( shop ) {
			...
			$.ajax({
					...
					$.each(data, function (i, value) {
						...
						li += '<div class="img"><img src="' + yyiRinkerEscapeHTML( value['<?php echo self::IMAGE_S_COLUMN ?>'] ) + '"></div>';
						li += '<div class="detail"><div class="title">'	+ yyiRinkerEscapeHTML( value['<?php echo self::TITLE_COLUMN ?>'] ) + '</div>';
						if ( value['<?php echo self::PRICE_COLUMN ?>'] !== '' ) {
                            if (shop === 'rakuten') {
                                li += '<div class="price">¥' + yyiRinkerEscapeHTML(yyiRinkerComma(value['<?php echo self::PRICE_COLUMN ?>'])) + '（アフィリエイト利率:' + yyiRinkerEscapeHTML(value['affiliateRate']) + '%）（レビュー平均:' + yyiRinkerEscapeHTML(value['reviewAverage']) + '）</div>';
                            } else {
                                li += '<div class="price">¥' + yyiRinkerEscapeHTML(yyiRinkerComma(value['<?php echo self::PRICE_COLUMN ?>'])) + '</div>';
                            }
                        }
						li += '<div class="button-box"><button class="button select add-items"';
						li += ' data-item-<?php echo self::TITLE_COLUMN ?>="' + yyiRinkerEscapeHTML( value['<?php echo self::TITLE_COLUMN ?>'] ) + '" ';
...
<?php if ( $tab === self::TAB_RAKUTEN ) { ?>
			params['<?php echo self::SEARCH_SHOP_VALUE ?>'] = '<?php echo self::SEARCH_SHOP_RAKUTEN ?>';
<?php } else { ?>
			params['<?php echo self::SEARCH_SHOP_VALUE ?>'] = '<?php echo self::SEARCH_SHOP_AMAZON ?>';
<?php } ?>
			...
			$.each(params, function( index, val ) {
				...
				switch ( index ) {
					case 'title':
						...
						break;
						case '<?php echo self::SEARCH_SHOP_VALUE ?>':
			...
			$.ajax({
				...
				$.each(data, function (i, value) {
					var li = '<li class="items">';
                        li += '<div class="img"><img src="' + yyiRinkerEscapeHTML( value['<?php echo self::IMAGE_S_COLUMN ?>'] ) + '"></div>';
                    	li += '<div class="detail"><div class="title">' + yyiRinkerEscapeHTML( value['<?php echo self::TITLE_COLUMN ?>'] ) + '</div>';
                        li += '<div class="links"><a class="button" href="' + yyiRinkerEscapeHTML( value['<?php echo self::AMAZON_URL_COLUMN  ?>'] ) + '" rel="nofollow" target="_blank">Amazon確認</a>';
                        li += '<a class="button" href="' + yyiRinkerEscapeHTML( value['<?php echo self::RAKUTEN_URL_COLUMN  ?>'] ) + '" rel="nofollow" target="_blank">楽天確認</a>';
                        li += '<a class="button" href="' + yyiRinkerEscapeHTML( value['<?php echo self::YAHOO_URL_COLUMN  ?>'] ) + '" rel="nofollow" target="_blank">Yahoo確認</a>';
